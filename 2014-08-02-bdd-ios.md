...
Starting you adventure with testing is not an easy task, especially if you don't have someone to help you out. You probably remember that moment when you've thought: this is it. I am starting testing now. I've heard so much about TDD and how beneficitial it is that I'm starting doing it right now. 

Then you sit down in front of your computer. You open your IDE. You create a new test file for one of your components.

And then void. You might write a few tests that check some basic methods but you feel that there's something wrong. You feel that there's a question lurking somewhere in your head. A question that needs answering before you can really unleash the power of testing. 

**What should I test?**

The answer to that question is not simple. In fact it is a quite complicated issue. But you still want to pursue the idea of having tests. So you write tests that just call your methods (hey, unit testing right?).

	-(void)testViewWillAppear;

Tests like this don't really tell you what should happen there. They don't tell what is actually being expected here. It is not **clear** what are the requirements. 

Moreover when you're refactoring and one of tests failed you have to dive into the code and **understand** why it failed. That requires a lot of additional unnecessary cognitive load. In an ideal world you shouldn't have to do that in order to understand what broke. 

This is where BDD comes it. It aims at solving these exact issues by helping developer determine *what* should be tested. Moreover it provides a DSL that allows developers to easily *clarify* their requirements and it introduces an ubiquitous language that helps easily *understand* what is the purpose of this test. 
...
We now know that we want to test the behaviors so in our tests we introduce following methods:

	-(void)testCarIsRed;
	-(void)testCarStarted;
	-(void)testCarMoveAndEngineStarted;
	-(void)testCarMoveAndEngineNotStarted
	
But what if we had more contexts that define whether our car can move? For instance introduce a new object `HandBreak`. We'd then end up with even more complicated tests and less readable suite.  BDD aims at helping developers solve this issue by introducing a DSL that clarify requirements:
...
First of all BDD DSL introduces a `describe` block that helps with clarifying requirements. It ties a certain part of tested functionality, for instance starting a car like in the example above. Each `describe` block consists of a name parameters that helps us undestand what is being test and a block that contains actual tests. You can have as many tests as you wish per describe block. Moreover there is no limit on nesting `describe` blocks. This allows us to write different specifications based on contexts in which the object (or its dependencies) is. For instance let's imagine that we're testing a method called `moveTo`:
...
Note how we leveraged nesting capabilities to provided two requirements based on different contexts (`context` block is a syntax sugar and it offers behavior similar to `describe`) in which our object could be. We also added two expectations to specifcy how our object should behave when it is in given context. 



Subjects of `describe` and `it` blocks are later used to construct names of examples, for instance `Car when it is started should have engine running`. 

	
`SpecBegin` declares a test class named `CarSpec`. `SpecEnd` closes that class declaration. 

`describe` block declares a group of examples that are tied to the subject (first parameter) of the block. 

`context` block behaves similary to `describe` (syntax sugar).

`it` is a single example (a single test). 
	
This syntax was originally introduced in JBehave[^1], a Java BDD framework, back in 2003. Even though example above has been adjusted to capabilities of Objective-C its principles still stand. 
...
This is why I strongly believe that view controllers should be tested as well. However testing view controllers is not an easy task. The Upload Photo and Login View examples should help with understanding how BDD can be leveraged to simplify building view controllers test suites.
...
I should give my thanks to [Robert Wijas](https://github.com/robertwijas) who came up with the idea of with traversing view hierarchy with predicates. 
