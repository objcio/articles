...
Behavior testing means that you are testing, well, behaviors. It means that you should only care about interface defined by your object: what methods and dependencies it has. 

These methods, these dependencies define *contract* of your object. They define how it should interact with the rest of your application. And that is what you should be aiming at: testing that interactions only, leaving other details apart. 
...
Second of all by testing interface of an object your tests become immune to change. This means they won't easily break when you exchange the implementation of an object. This a desired behavior as you should be able to perform refactor without modyfing your tests (or with minimal change). If your tests would be susceptible to change you would also have to refactor your test suite. And again, we're all lazy. 

Last, but not least, behavior tests force you to think *how* your object should behave before you actually write it. 

The key aspect of writing good behavioral tests is identifying dependencies (you can read more on this subject [here](link to Jons article)). The reason for this is that most of your tests will either assert whether a specific interaction happened, or whether a specific value was returned (or passed in to another object) based on your tested object state. 

Every unit test always consists of three stages:

* Arrange (Prepare input data, either by providing it via stubbed dependencies or as parameters)
* Act (simulating execution of the behavior that is being tested)
* Assert (Check whether output data that was returned from the method)

Let's take a look at a few examples of how can we test behaviors of components. 
...
When I writen an with TDD in mind I always have a Terminal window opened. In that window I execute a script that listens for file changes in folder with source code that I'm currently working on. Whenever a change is detected scripts runs `xcodebuild test` automatically. That way I almost instantly get feedback about changes I've just introduced. 

The only problem with above trick is that output from `xcodebuild test` is not exactly readable, especially when you test suite grows large. Imagine what happens when your app reaches 2000 (or more) tests - finding which test failed would become quite a challange.

This is where [xcpretty](https://github.com/supermarin/xcpretty) comes in. It is a really simple tool - the only thing it actually does is that it parses output Xcode throws out when performing `xcodebuild test`. What is great is that you get a green `.` for a passed test and a red `F` for a failed one. Plus all info you can get about a failed test. But it gets even better - since xctool doesn't print all information `xcodebuild` gives it it actually *faster* than running tests normally. 
