...
Our mocking framework is [OCMock](http://ocmock.org/). As described in the article about [mocking](/issue-15/mocking.html) in this issue, a mock is an object that returns a prepared answer for method calls.
...
Since we're relying heavily on code data, we added a method to `NSManagedObjectContext` called
...
This works, because `-tearDown` gets called on the main loop. We spin the main loop to make sure that any code that might get enqueued onto the main queue gets to run. The above code will hang indefinitely if the group never empties. In our case, we adapted it slightly, so that we have a timeout.
...
OS X Server has a feature called [Xcode Server](https://www.apple.com/osx/server/features/#xcode-server), which is Continuos Integration server based on Xcode. We have been using this.
