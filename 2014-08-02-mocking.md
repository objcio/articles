...
### OCMock
OCMock is another Objective-C mock object library. Like OCMockito, it provides full functionality for stubs, mocks, and just about everything else you might want. It has a lot more functionality than OCMockito, which depending on your personal preference might be a benefit or a drawback.
...
### Which to use?
Like all things related to testing, what approach you take depends completely on both the specifics of what you're testing and your own personal preference. OCMockito and OCMock are both installable via CocoaPods, so they're easy to integrate with your existing test setup, but there is also something to be said for avoiding adding dependencies. 

## Things to watch out for while mocking

### Don't mock what you don't own
One of the biggest problems you run into with testing is writing tests that are too tightly coupled to the implementation of your code. The entire point of testing is reducing the cost of future change; if changing the implementation details of some of your code breaks your tests, you've increased that cost.

If this is a problem with your own code, it's doubly so a problem with code that's not yours. With your codebase, you probably have a sense of how stable or volatile different interfaces are; you generally have no such guarantee with third-party code. As a result, there's a classic idiom that you "shouldn't mock what you don't own".

Whether or not that's true comes down, I think, to whether you're a mockist or a statist.
Sometimes the only solution to avoid mocking out a third-party dependency is to create your own higher-level class that abstracts away that behavior so that you can mock out your own class. Sometimes this is the right choice, but it often amounts to little more than shifting your complexity elsewhere without decreasing it meaningfully. While "don't mock what you don't own" is a great guideline, what's worth keeping in mind is that there is no silver bullet in testing, and what works great in one situation might be more trouble than it's worth in another.

The other reason to not mock out third-party dependencies is more philosophical. Part of the reason to write tests using mocks — as in, objects that verify behavior — is to help drive out what the interface between your two objects should look like. But with a third-party dependency, you don't have control in that; the specifics of the API contract are already set in stone by a third party, so you can't effectively use tests as an experiment to see if it could be improved. This isn't a problem, per se, but in many cases it reduces the effectiveness of mocking to the point that it's no longer worth it.

#### In Objective-C
What complicates things even more is the uniqueness of the iOS and OS X development world. So much of what we do is dependent on first-party frameworks, which tend to be more over-reaching than the standard library in many other languages. Although NSUserDefaults is an object you "don't own", for example, if you find yourself needing to mock it out it's a fairly safe bet that Apple won't introduce breaking API changes in a future Xcode release.
