...
Now that we have setup our camera with 30 frames per second frame rate, our implementation of `processImage:` will be called 30 times per second. Since our app will detect faces continously, we should perform our face detection here. Please note that if the face detection at each frame take longer than 1/30 seconds, we will be dropping frames. 
...
````
Please note that we had to convert from `UIImage` to `cv::Mat` through a category method. Additionally the OpenCV face recognizer only supports integers as labels but we would like to be able to use a person's name as a label and had to implement a simple conversion between them.
