...
Few have heard of DTrace. It's a little gem tucked away somewhere inside the OS. It's a debugging power tool. Powerful because it's extremely flexible. And probably relatively unknown because it is so very different from other tools out there.

Quite often real users or testers of your app will see some unexpected behaviour. DTrace lets you answer arbitraty questions about the app -- on a production version of the app, and without re-launching the app.



## Dynamic Tracing

Almost ten years ago [Sun Microsystems](https://en.wikipedia.org/wiki/Sun_Microsystems) built DTrace -- short for *Dynamic Trace*. And at the end of 2007 Apple integrated it into their [operating system](https://en.wikipedia.org/wiki/Mac_OS_X_Leopard).

DTrace is a dynamic tracing framework which provices *zero disable cost*, ie. probes in code have no overhead when disabled — we can leave the probes in our code even for production builds. You only pay the cost of the probe when using it.
...
In this article we’ll focus on using DTrace to investigate our own program, but it’s worth mentioning that DTrace is system wide: A single script could eg. watch allocations made by *all* processes on the system. Take a look inside `/usr/share/examples/DTTk` for some great examples.
...
At Zeta [TODO] DTrace has been very helpful to us on iOS, too, even though we're limited to use it in the iOS simulator. If you're reading this article, and think real DTrace support on iOS devices is a good idea, please file an [enhancement request](https://bugreport.apple.com/) with Apple.
...
There are build-in probes, and you can add (so called static) probes to your code. A probe looks very similar to a normal C function. At Zeta [TODO] our syncing code has in internal state machine, and we define these two probes

    provider syncengine_sync {
        probe strategy_go_to_state(int);
    }

Probes are grouped into so called *providers*. The `int` argument is the state that we're entering. In our Objective-C (or Swift) code, we simply insert
...
etc. as we'd expect. Nothing too exiciting. Hit `^C` to exit DTrace.
...
If we extend the trivial example from above, we can output the time spend in each state:
...
We also added a predicate `/ last_state_timestamp != 0 /` to the 1st probe.
...
There's another timestamp variable, `vtimestamp`, which is a virtualized timestamp, also in nanoseconds. It is the amount of time the the current thread had been running on a CPU, minus the time spent in DTrace. Finally machtimestamp corresponds to `mach_absolute_time()`.

For the above script, the order of execution is important. We have 2 so-called *clauses* which match the same probe (`syncengine_sync*:::strategy_go_to_state`). These will run in the order in which they appear in the D program.

### Combining with System Provided Probes

The operating system, particularly the kernel, provides throusands of probes, grouped into various providers. Quite a lot of these are the ones documented by [Oracle's DTrace documentation][oracleDTraceProviders].
...
This time we are targeting a specific process -- the `ip:::send` would otherwise match all processes on the system while we're only interested in the `Zeta` process. We run this script with

    sudo dtrace -q -s sample-timing-3.d -p 198

where `198` is the process identifier (aka. PID) of the process. We can find this number in the Activity Monitor app, or using the [`ps(1)` command line tool][ps1man].
...
In the above examples, we have seen a few of these *probe clauses*. They all follow the general form
...
The probe desciption defines what probe the clause matches. These are all of the form

    provider:module:function:name

where parts can be left out. E.g. `syscall:::` matches all probes by the `syscall` provider. We can use `*` to match any string such that `syscall::*lwp*:entry` matches all `entry` probes by the `syscall` provider where the function name contains `lwp`.

A probe description can consist of multiple probes, e.g.
...
We can list the same probe multiple times with different predicates. If multiple of them match, they will be run in the order that they appear in the D program.

### Actions

The actions are enclosed in curly braces. The D language is lightweight, small and simple.

D does not support any control flow such as loops or branches. We can not define any user functions. And variable declaration is optional.

This limits what we can do. But the simplicity also gives us a lot of flexibility once we've know a few common pattern which we'll look into now. We will look at this in the next section. Be sure to also check out the [D Programming Language][DProgrammingLanguage] guide for details.
...
If we run this and launch Mac *App Store* application, then exit the D Trace script with `^C`, we will get output like this:
...
The *App Store* spend 3.6 seconds CPU time inside the `kevent64` syscall etc.
...
In the scrip we use the first probe clause to match when we enter a syscall. We set the thread local variables `self->ts` to the current timestamp:
...
We add `self->ts != 0` to the predicate to be sure that our script work, even if we attach to the application while it is inside a system call. Otherwise `timestamp - self->ts` would result in a very large value because `self->ts` would not have been set.
...
uses aggregations. This is an extremely powerful feature of DTrace.
...
The [DTrace Guide](https://docs.oracle.com/cd/E19253-01/817-6223/chp-aggs-trunc/index.html) shows this small example script which uses aggregations to print the number of system calls per second of the ten applications that do the most system calls:
...
On a mostly idle OS X it may show something like
...
The total list of aggregation functions is
...
The `quantize` and `lquantize` functions can be very helpful to get an overview of what quantities are being passed.
...
Despite the name, arrays in D are more akin to dictionaries in Swift or Objective-C. And in addition, they are multi-dimensional.

We can define an associative array with
...
For the Zeta app, we want to track the round trip times for [NSURLSessionTask][AppleDocNSURLSessionTask] instances. We fire a [statically defined probe](#staticProbes) when we resume such a task, and a second probe when it completes. With that we can write a simple script like this
...
Associative arrays are also very helpful to provide a good description for `enum` values passed into a clause, as we've seen in the [first example “A Timing Example” above](#ATimingExample).

## Probes and Providers

Let us take a step back and look at the probes that are available.

We can get a list of all available providers with
...
A few of these providers are part of the original set documented in the [Dynamic Tracing Guide][oracleDTraceProviders]. Let's look at a few of those that are available to us:

### `dtrace` Provider

We mentioned the `BEGIN` and `END` probes [above](#ATimingExample). The `dtrace:::END` is particularly helpful for outputting summaries when running DTrace in the quiet mode. There's an `ERROR` probe, too, for when an erro occurs.

### `profile` Provider

The [`profile` provider][profileProvider] can be used for sampling in a way very that should be familiar to users of Intruments. 
...
Similarly the `tick-` probes will fire every fixed interval at high interrupt level. The `profile-` probes will fire on all  CPUs, the `tick-` probes only on a single CPU per interval. We used `tick-10sec` in our [aggregation](#Aggregations) example above.
...
`pid` is short for process identifier. It lets us probe on function entry and exit of a process. This works in *most* cases. Note that function return and entry is not always well defined, particularly due to *tail-call optimization*. And some functions don’t have to create a stack-frame, etc.

When you can't change the code to add [static probes](#staticProbes), `pid` is a powerful tool.

You can trace any function that's visible. For example the probe

    pid123:libSystem:printf:return

would attach to returning from `printf` in the process with the process identifier (pid) 123.

### `objc` Provider

A direct counter part to the `pid` provider is the `objc` provider. It provides probes for Objective C method entry and exit. Again, using [static probes](#staticProbes) provides more flexibility.

The format for `objc` probe specifiers is
...
would match entry into all instance methods of `NSTableView` in the process with pid 207. Since the colon symbol `:` is part of the DTrace probe specifier scheme, `:` symbols in Objective C method names need to be replaced with a questionmark `?`. E.g. to match entry into `-[NSDate dateByAddingTimeInterval:]` we'd use
...
The sample from the [Oracle documentation][ioProvider] show how this can be put to use:
...
There are two probes `send` and `receive` inside the the [`ip` provider][ipProvider]. They are triggered whenever data is either send or received over IP. The arguments `arg0` to `arg5` provde access to the kernel structures related to the IP packet being sent or received.

This can be put together into very powerful network debugging tools. It will make using `tcpdump(1)` seem like yesteryear's trick. The `ip` provider let's you print exactly the information you are interested in and at the point in time when you're interested in it.
...
These kind of probes are called *static probes* in DTrace. We briefly talked about this in the [first examples](#ATimingExample). The [Zeta app](TODO: Link) defines its own provider with it's own probe:
...
One might argue that we could just have used the `objc` provider, it should be obvious that using our own probe gives us more flexibility: We can later change the Objective C code without afecting the DTrace probes.
...
The power of DTrace static probes is that it gives us a stable interface to debug our code. And an interface that exists even in production code. When someone is seeing odd behaviour, we can attach a DTrace script to the running app, even for a production build of that app. And the flexibility of DTrace allows us to use the same probes for multiple purposes.
...
Let us take a look at how we add static probes to our project:

### Provider Description

First we need to create a `.d` file that defines the providers and probes. If we create a `provider.d` file with this content
...
we will have two providers: `syncengine_sync` and `syncengine_ui`. Within each we define a set of probes with their argument types.

### Creating a Header File

We now need to add the `provider.d` file to an Xcode target. It is important to make sure that the type is set to *DTrace source*. Xcode now automatically processes it when building. During this processing, DTrace creates a corresponging `provider.h` header file that we can include. It is important to add the `provider.d` file to both the Xcode project and the corresponding target.

Under the hood Xcode calls the `dtrace(1)` tool:

    dtrace -h -s provider.d

which will generate the corresponding header file. The header file ends up in the [`DERIVED_FILE_DIR][XcodeBuildDerivedFileDir] and hence can be imported with

    #import "provider.h"

in any source file in the project. Xcode has a built-in so called *build rule* for DTrace provider descriptions. C.f. the [objc.io issue #6 “The Build Process”](/issue-6/build-process.html) for more information on build rules and the build process in general.
...
The DTrace probes themselves are zero cost when not enabled, ie. as long as no one attached to a probe, they're for free. Sometimes, however, we need want to precalculate / preprocess some data before sending it to a probe. In those rare cases, we can use the `_ENABLED()` macro like so:
...
Code readability is very important. As we add more and mroe DTrace probes to our code, we need to make sure the code doesn't get cluttered and incomprehensible due to the probes. After all the probes are here to help us, not to make things more difficult.
...
If we go back to the state machine example, our probe macro is
...
we defined above, we have a long list of arguments. In the Zeta app [TODO] we use this to log UI notifications.

This probe as a very long list of arguments. We decided that we wanted to use a single probe for a lot of different notifications that the sync'ing code sends to the UI to notify it about changes. The first argument `arg0` specifies what the notification is, and the second one the `object` of the `NSNotification`. This makes it easy for our DTrace scripts to narrow in on those sepcific notifications of interest in the given situation.

The remainder of the arguments are less loosly defined on a per-notification bases, and we have multiple wrapper functions for the individual cases. When we want to pass two `NSUUID` instances, we call a wrapper function like this:
...
This wrapper function is defined as
...
As noted, this serves two purposes, for one, we don't clutter our code with things like `(intptr_t) (__bridge void *)`, and secondly we don't spend CPU cycles converting an `NSUUID` to an `NSString` and then to a `char const *` unless we need to because someone is attached to the probe.
...
The D programming language is a compiled language. When we run the `dtrace(1)` tool, it compiles the script we pass to it into byte code. This byte code is then passed down into the kernel. Inside the kernel there's an interpreter that runs this bytecode.
...
It should be obvious that DTrace is extremely powerful and flexible. It is important to note, though that DTrace is not a replacement for tried and true tools such as `malloc_history`, `heap`, etc. As always: use the right tool for the job at hand.
...
If you have code that has `#ifdef TRACING` or `#if LOG_LEVEL == 1` in it, these are probably good candidates for code to be replaces by DTrace.

Be sure to check out the [Dynamic Tracing Guide][DynamicTracingGuideHTML] ([PDF version][DynamicTracingGuidePDF]). And peek into your systems `/usr/share/examples/DTTk` directory for some inspiration.
