...
However, there are cases when these projects are useful, such as with a custom UI like [Clear][]. UIKit simply doesn't provide access to the correct APIs to programmatically perform the gestures required to test Clear's UI.
...
Not all controls in UIKit have an equivalent method to `sendActionsForControlEvents:`, but with a bit of creativity, it's often possible to find a workaround. As said before, the most important thing is to make it appear to your code as if the user triggered it.
...
As a final example of testing controls, I want to present the special case of `UIBarButtonItem`s. They don't have a `sendActionsForControlEvent:` method because they're not descendents from `UIControl`. Let's figure out how we can send the button action and, to our code, make it look like the user tapped it.
